	...
	listen : function(socket){
		...
		socket.on('connection', function(client){
			client.on('message', function(message){
	    		var response = self.handleMessage( JSON.parse(message),client);
    ...
    handleMessage: function(message,client) {
        ...
        var resp = {
            ...
            sendError : function (error){
                JSONRPC.trace('-->', 'error (id ' + message.id + '): ' + error);
                var result = { 'id': message.id,
                    'error': error };
...
//	    try {
	      var resp = method.call(resp, message.params);
